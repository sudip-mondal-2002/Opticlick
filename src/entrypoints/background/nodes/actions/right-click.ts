...
import { dispatchRightClick } from '@/utils/cdp';
...
type RightClickAction = Extract<AgentAction, { type: 'right_click' }>;
...
export async function handleRightClick(
  ...
  await log(`Right-clicking element #${target.id} "${target.text}" at (${target.rect.x}, ${target.rect.y})`, 'act');

  try { await sendToTab(tabId, { type: 'UNBLOCK_INPUT' }); } catch { /* */ }

  try {
    await dispatchRightClick(tabId, target.rect.x, target.rect.y);
  } catch (err) {
    const errMsg = (err as Error).message;
    await log(`Right-click on #${target.id} failed: ${errMsg}`, 'warn');
    await appendConversationTurn(
      sessionId, 'tool',
      `[ACTION FAILED - Step ${step}] Right-clicking element #${action.id} ("${target.text}") failed: "${errMsg}". Task: ${userPrompt}`,
      { toolCallId, toolName },
    );
    await sleep(STEP_DELAY_MS);
    return tabId;
  ...
  await appendConversationTurn(
    sessionId, 'tool',
    `[Step ${step}] Right-clicked element #${action.id} ("${target.text}"). Task: ${userPrompt}`,
    { toolCallId, toolName },
  );
