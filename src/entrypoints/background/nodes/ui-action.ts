...
import { handleDoubleClick } from "./actions/double-click";
import { handleRightClick } from "./actions/right-click";
import { handleHover } from "./actions/hover";
...
export async function uiActionNode(
  ...
  const uiAction = actions.find((a) =>
    noElements
      ? UI_ACTION_TYPES_NO_CLICK.has(a.type)
      : UI_ACTION_TYPES.has(a.type),
  );
  ...
  if (uiAction.type === "scroll") {
    const newHistory = await handleScroll(
      uiAction,
      ctx,
      actionHistory,
      coordinateMap,
  ...
  if (uiAction.type === "double_click") {
    tabId = await handleDoubleClick(uiAction, ctx, coordinateMap, tabIdRef);
    return { tabId };
  }

  if (uiAction.type === "right_click") {
    tabId = await handleRightClick(uiAction, ctx, coordinateMap, tabIdRef);
    return { tabId };
  }

  if (uiAction.type === "hover") {
    await handleHover(uiAction, ctx, coordinateMap);
    return { tabId };
  }
